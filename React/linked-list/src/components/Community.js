class City extends Object {
	...
	show() {
		return this.name + ' ' + this.lat.toString() + ' ' + this.long.toString() + ' ' + this.pop.toString();
	}
...
class Community extends Object {
	...
	addCity(name, latitude, longitude, population) {
		...
		let tmpCity = new City(name, latitude, longitude, population);
		if (length > 0) {
	...
	showAll() {
		...
		for (x = 0; x < this.myCities.length; x++) {
			showall += `name:${this.myCities[x].name} lat:${this.myCities[x].lat} long:${this.myCities[x]
				.long} pop:${this.myCities[x].pop}
        \n`;
		}
		return showall;
	}
